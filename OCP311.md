...
* [ ] Once routes are confirmed, submit a change request to add the routes to the DNS.  This enables the communication between the services without the use of wildcards.

 * [ ] Post installation and confirmation of the install, execute the following commands to ensure proper access.
  If not already logged in as admin:  
	   execute “oc login -u system:admin”

  Bind the ocadmin to the cluster admin role:
	   execute “oc create clusterrolebinding registry-controller --clusterrole=cluster-admin --user=ocadmin”
	   execute “oc adm policy add-cluster-role-to-user cluster-admin ocadmin”
